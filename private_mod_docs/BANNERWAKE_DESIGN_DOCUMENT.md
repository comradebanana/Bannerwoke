# BANNERWAKE DESIGN DOCUMENT

Clean-install status review generated 2026-06-03.

This is the current code-facing review of the source design document, not a replacement for its creative content.
Source design document: `C:\Users\korom\Downloads\BANNERWAKE_DESIGN.txt`
Source length: 3744 lines.

## Current Baseline

- Game root: `C:\Program Files (x86)\Steam\steamapps\common\Mount & Blade II Bannerlord`
- Documents/config root: `C:\Users\korom\Documents\Mount and Blade II Bannerlord`
- Vortex staging: `C:\Users\korom\AppData\Roaming\Vortex\mountandblade2bannerlord\mods`
- Vortex downloads: `C:\Users\korom\AppData\Roaming\Vortex\downloads\mountandblade2bannerlord`
- Steam Workshop downloads: `C:\Program Files (x86)\Steam\steamapps\workshop\downloads\261550`
- Steam Workshop content: `C:\Program Files (x86)\Steam\steamapps\workshop\content\261550`
- Practical target remains `v1.4.5.114824` because LauncherData/core modules report v1.4.5 and NavalDLC requires v1.4.5.
- `package_info.txt` still reports `Environment: PC@v1.3.4`; treat that as an install/build metadata quirk.

## Outdatedness Verdict

The design document is creatively still strong, but these implementation assumptions now need clean-install corrections:

- Installed/deployed is not the same as active. `LauncherData.xml` currently selects: Native, SandBoxCore, BirthAndDeath, CustomBattle, Sandbox, StoryMode, NavalDLC, BannerKings, RBM, RBM_WS, CalradiaRisingArmory.
- Vortex has deployed many modules into the game directory, but Vortex load-order cache files currently mark all listed entries as `isSelected: false`.
- Steam Workshop `content\261550` now contains installed Workshop modules. `workshop\downloads\261550` also retains staged/download payloads.
- TAOM/Tales from the Age of Men is now a large deployed module cluster and has contributed/overwritten framework modules.
- Framework versions in deployed `Modules` are lower than some installed mods declare. Reconcile Harmony, ButterLib, UIExtenderEx, and MCM before runtime tests.
- Banner Kings is installed. Keep the original warning as a hard rule: study specific systems only; do not make Bannerwake depend on it.
- Religions is installed but declares old `e1.6.0` base dependencies. Treat it as source study only.
- Fire Arrows is now installed in Steam Workshop content as `FireArchers`; Fire Swords Polearms and Axes is still not detected locally.
- Party AI Overhaul / Party AI Controls are not detected locally.
- Calradia Rising Armory is active from Steam Workshop content, not from the game `Modules` folder.
- BannerKings is currently selected while its required framework dependencies are not selected.

## Mentioned Mod Local Status

| Reference                      | Local status           | Judgement                                                                                                      |
| ------------------------------ | ---------------------- | -------------------------------------------------------------------------------------------------------------- |
| Open Source Armory             | Modules                | Present; split cleanly with OSW/OSS. Primary asset pool is now available.                                      |
| Open Source Weaponry           | Modules                | Present; not just an abstract recommendation anymore.                                                          |
| Open Source Saddlery           | Modules                | Present; mounted/coastal/elite cavalry visual source now available.                                            |
| Swadian Armoury                | Modules                | Present with corrected Nexus 2349 identity. Folder id contains a space and lowercase name.                     |
| Hallow Plate Armor             | Modules                | Present as a separate optional armor source; do not confuse with Swadian Armoury.                              |
| Terra Armarium                 | Missing                | Not found in Modules, Vortex downloads, or Steam staged module list.                                           |
| Fire Swords Polearms and Axes  | Missing                | Not detected locally. The Undying Flame melee-fire reference still needs local install if you want it studied. |
| Fire Arrows                    | Steam Workshop content | Installed in Steam Workshop content as FireArchers; currently visible in LauncherData but not selected.        |
| Siege Engines Extended         | Missing                | Not detected. Siege-system reference remains web/design-only for now.                                          |
| Adventurer                     | Modules                | Present as deployed module v1.2.10.                                                                            |
| Titles                         | Missing                | Not detected locally.                                                                                          |
| Champion's Relics              | Modules                | Present as deployed module.                                                                                    |
| Horde Kingdom                  | Steam Workshop content | Installed in Steam Workshop content as HordeKingdomMod; currently visible in LauncherData but not selected.    |
| Immersive Storms               | Steam Workshop content | Installed in Steam Workshop content as ImmersiveStormsMod; currently visible in LauncherData but not selected. |
| Esoteric Knowledge             | Missing                | Not detected locally.                                                                                          |
| AI Influence                   | Steam downloads only   | SubModule exists in Steam download staging but no installed Modules folder.                                    |
| Special Troops Plus            | Steam Workshop content | Installed in Steam Workshop content as SpecialTroopsPlus; currently visible in LauncherData but not selected.  |
| Calradia At War                | Modules                | Present and paired with CustomSpawns.                                                                          |
| CustomSpawns                   | Modules                | Present but declares Harmony v2.4.2; current deployed Harmony is lower.                                        |
| No More Idle Minor Factions    | Modules                | Present. Supports the home-settlement/drift rule.                                                              |
| Religions                      | Modules                | Present, but declares old e1.6.0 base dependencies. Treat as source-study only until verified.                 |
| Diplomacy                      | Modules                | Present with War Sails patch, but framework dependency versions are currently lower than declared.             |
| WarAndAiTweaks                 | Missing                | Not detected locally.                                                                                          |
| Bellum Civile                  | Missing                | Not detected locally.                                                                                          |
| Realistic Battle Mod           | Modules                | Present with RBM_WS War Sails submod.                                                                          |
| DynaCulture                    | Missing                | Not detected locally.                                                                                          |
| Cultural Identity              | Modules                | Present.                                                                                                       |
| Banner Kings                   | Modules                | Present. Design warning should be stronger: study only, do not depend on for Bannerwake core.                  |
| Tales from the Age of Men      | Modules                | Present as large deployed LOTR/TAOM module cluster and asset source; do not enable casually for Bannerwake.    |
| Party AI Overhaul and Commands | Missing                | Not detected locally.                                                                                          |
| Party AI Controls              | Missing                | Not detected locally.                                                                                          |
| Better Exception Window        | Modules                | Present, but deployed SubModule is v7.1.1 while Vortex download says v8.0.0.                                   |
| BLSE                           | Root/bin deployment    | Present via vortex.deployment.bannerlord-blse.json, not a normal module.                                       |

## Dependency Red Flags

| Kind    | Module                            | Dependency                | Requested | Required? | Installed  |
| ------- | --------------------------------- | ------------------------- | --------- | --------- | ---------- |
| LOWVER  | Bannerlord.Diplomacy              | Bannerlord.Harmony        | v2.4.2    | required  | v2.3.6.220 |
| LOWVER  | Bannerlord.Diplomacy              | Bannerlord.ButterLib      | v2.10.2   | required  | v2.10.0    |
| LOWVER  | Bannerlord.Diplomacy              | Bannerlord.MBOptionScreen | v5.11.3   | required  | v5.11.2    |
| LOWVER  | Bannerlord.Diplomacy              | Bannerlord.UIExtenderEx   | v2.13.2   | required  | v2.13.0    |
| LOWVER  | Bannerlord.DiplomacyNavalDLCPatch | Bannerlord.Harmony        | v2.4.2    | required  | v2.3.6.220 |
| LOWVER  | Bannerlord.DiplomacyNavalDLCPatch | Bannerlord.ButterLib      | v2.10.2   | required  | v2.10.0    |
| LOWVER  | Bannerlord.DiplomacyNavalDLCPatch | Bannerlord.MBOptionScreen | v5.11.3   | required  | v5.11.2    |
| LOWVER  | Bannerlord.DiplomacyNavalDLCPatch | Bannerlord.UIExtenderEx   | v2.13.2   | required  | v2.13.0    |
| LOWVER  | Bannerlord.DiplomacyNavalDLCPatch | NavalDLC                  | vv1.3.4   | required  | v1.2.5     |
| LOWVER  | CustomSpawns                      | Bannerlord.Harmony        | v2.4.2    | required  | v2.3.6.220 |
| MISSING | RBM                               | RTSCamera                 |           | optional  |            |
| MISSING | RBM                               | RTSCamera.CommandSystem   |           | optional  |            |
| MISSING | RBM_WS                            | RTSCamera                 |           | optional  |            |
| MISSING | RBM_WS                            | RTSCamera.CommandSystem   |           | optional  |            |
| LOWVER  | Religion                          | Native                    | e1.6.0    | required  | v1.4.5     |
| LOWVER  | Religion                          | SandBoxCore               | e1.6.0    | required  | v1.4.5     |
| LOWVER  | Religion                          | Sandbox                   | e1.6.0    | required  | v1.4.5     |
| LOWVER  | Religion                          | StoryMode                 | e1.6.0    | required  | v1.4.5     |

## Active Load-Order Red Flags

| Kind         | Active module | Dependency                | Requested | Problem                                                          |
| ------------ | ------------- | ------------------------- | --------- | ---------------------------------------------------------------- |
| DEP_INACTIVE | BannerKings   | Bannerlord.Harmony        |           | Required dependency is present but not selected in LauncherData. |
| DEP_INACTIVE | BannerKings   | Bannerlord.ButterLib      |           | Required dependency is present but not selected in LauncherData. |
| DEP_INACTIVE | BannerKings   | Bannerlord.UIExtenderEx   |           | Required dependency is present but not selected in LauncherData. |
| DEP_INACTIVE | BannerKings   | Bannerlord.MBOptionScreen |           | Required dependency is present but not selected in LauncherData. |

## Duplicate Module IDs Across Roots

| Module ID          | Modules version/path                                                                                              | Workshop version/path                                                                |
| ------------------ | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| Bannerlord.Harmony | v2.3.6.220 | C:\Program Files (x86)\Steam\steamapps\common\Mount & Blade II Bannerlord\Modules\Bannerlord.Harmony | v2.4.2.0 | C:\Program Files (x86)\Steam\steamapps\workshop\content\261550\2859188632 |

## Still Canonical Design Decisions

- No separate spawn-budget layer unless you later reverse that decision. Use native party templates, home settlements, state gates, naval party logic, and event escalation.
- No lite pass for non-Vlandian factions. Every major faction and sub-faction receives full-depth treatment.
- NavalDLC remains a hard dependency and the correct sea-layer foundation.
- Black Keel and Vrak still need pseudo/custom kingdom handling when they own settlements.
- Vrak defeat should remain defeat/rout/exile/capture/exposure logic, not death logic.
- Design language should stay: lore becomes mechanics, not lore pasted onto mechanics.

## Immediate Design Patches To Make Later

1. Add a short clean-install baseline section to the source design doc pointing to these three new files.
2. Decide whether TAOM/LOTR modules are only private asset reference or whether their framework/module folders should be removed from the test stack.
3. Re-deploy the dependency stack in Vortex after TAOM if you want current Harmony/ButterLib/UIExtender/MCM versions to win.
4. Install/verify missing references if Claude is expected to inspect them locally.
5. Activate a deliberate launcher profile before any runtime test; the current launcher config is native-plus-NavalDLC only.
